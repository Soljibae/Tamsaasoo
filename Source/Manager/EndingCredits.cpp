...
namespace Manager
{
	...
	void EndingCredits::Update()
	{
        ...
        if (bIsPressedAnyKey)
        {
            Manager::gm.SetNextGameState(EGameState::MAINMENU);
        }
        else
        {
            EndingImage->size.x = global::ScreenWidth;
            EndingImage->size.y = global::ScreenHeight;
	...
	void EndingCredits::Draw()
	{
       Utils::DrawObject(*EndingImage, false,1.f);
       Utils::DrawObject(*PressAnyKeyImage, false, PressAnyKeyImage->Alpha);
	}
	void EndingCredits::Destroy()
	{
        if (EndingImage->Mesh)
        {
            Utils::DestroyMesh(EndingImage->Mesh);
        }
        if (EndingImage->Texture)
        {
            AEGfxTextureUnload(EndingImage->Texture);
        }
